...
amp_gui_gtk.so: amp_gui_gtk.c $(BUNDLE)
	gcc $< -o $@ -shared -fPIC -DPIC `pkg-config --cflags --libs gtk+-2.0`
	cp $@ $(BUNDLE)

amp_gui_fltk.so: amp_gui_fltk.cpp $(BUNDLE)
	g++ $< -o $@ -shared -fPIC -lcairo -I/usr/include/ntk -lntk -Wl,-z,nodelete
	cp $@ $(BUNDLE)
...
amp_gui_qt.so: amp_gui_qt.cpp amp_gui_qt.moc.cpp $(BUNDLE)
	g++ $< -o $@ -shared -fPIC -Wl,--no-undefined `pkg-config --cflags --libs QtCore QtGui`
	cp $@ $(BUNDLE)
